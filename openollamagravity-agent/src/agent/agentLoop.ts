...
export class AgentLoop {
  ...
  async run(
    ...
    for (let step = 1; step <= maxSteps; step++) {
      if (signal.aborted) { this.emit({ type: 'done', content: 'Stopped.' }); this.running = false; return; }
  ...
  private async _streamWithTimeout(step: number, totalSteps: number, signal: AbortSignal): Promise<string> {
    ...
    const firstTokenTimeoutMs = cfg.get<number>('firstTokenTimeoutSec', 90) * 1000;
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      const timeoutError = () => {
        reject(new Error(
            `Ollama не відповідає. Можливо, контекст завеликий. Спробуйте очистити чат або перезапустити Ollama.`
        ));
      };
  ...
  private async executeTool(name: string, args: any): Promise<Tools.ToolResult> {
    const confirm = async (label: string) =>
        (await vscode.window.showWarningMessage(`OOG: ${label}`, 'Allow', 'Deny')) === 'Allow';
