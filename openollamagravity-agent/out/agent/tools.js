...
function managePlan(args, state) {
    ...
    if (action === 'create') {
        if (!task)
            return { ok: false, output: 'Error: Missing "task" for create action.' };
    ...
    if (action === 'complete') {
        if (id === undefined)
            return { ok: false, output: 'Error: Missing "id" for complete action.' };
    ...
    if (action === 'delete') {
        if (id === undefined)
            return { ok: false, output: 'Error: Missing "id" for delete action.' };
...
async function openFileInBrowser(args) {
    try {
        if (!args.path)
            return { ok: false, output: 'Помилка: вкажіть "path".' };
        const abs = resolvePath(args.path);
